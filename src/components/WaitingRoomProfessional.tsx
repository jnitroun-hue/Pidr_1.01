...
export default function WaitingRoomProfessional({
  ...
  // ⚡ WEBSOCKET РЕАЛ-ТАЙМ ОБНОВЛЕНИЕ ЧЕРЕЗ SSE
  useEffect(() => {
    if (!roomData?.id) return;

    console.log('⚡ Подключаем SSE для комнаты:', roomData.id);

    let eventSource: EventSource | null = null;

    const connectSSE = () => {
      try {
        eventSource = new EventSource(`/api/rooms/${roomData.id}/ws`);
        ...
        eventSource.onmessage = (event) => {
          try {
            ...
            if (update.type === 'room_update') {
              console.log('⚡ Получено realtime обновление:', update);

              // Преобразуем данные в формат Player
              ...
              const playersChanged = updatedPlayers.length !== roomData.players.length ||
                updatedPlayers.some((p, idx) => {
                  const existing = roomData.players[idx];
                  return !existing || p.id !== existing.id || p.isReady !== existing.isReady;
                });

              const maxPlayersChanged = update.maxPlayers !== roomData.maxPlayers;

              if (playersChanged || maxPlayersChanged) {
                const updateData: any = { players: updatedPlayers };
                
                if (maxPlayersChanged) {
                  updateData.maxPlayers = update.maxPlayers;
                  console.log(`⚡ maxPlayers обновлен: ${roomData.maxPlayers} → ${update.maxPlayers}`);
                }
  ...
  }, [roomData?.id, roomData?.players.length, roomData?.hostId, roomData?.maxPlayers, onUpdateRoom]);
