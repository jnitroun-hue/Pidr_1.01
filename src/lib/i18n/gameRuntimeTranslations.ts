...
const exactRuToEn: Record<string, string> = {
  ...
  'В начале у каждого 1 открытая карта и 2 закрытые — это пеньки. Они лежат до 3 стадии, рубашкой вверх рядом с аватаром.\n\n🔒 Пеньки откроются только когда закончится колода И у вас не останется карт на руке.\n\n⚠️ Пока колода не кончилась — пеньки НЕ открываются, даже если рука пуста!\n\n💡 Пеньки могут быть как удачными, так и нет — это элемент интриги!':
    'At the start each player has 1 face-up card and 2 face-down stumps. Stumps stay closed until stage 3, face down near the avatar.\n\n🔒 Stumps open only when the deck is empty AND you have no cards in hand.\n\n⚠️ Until the deck is over, stumps do NOT open even if your hand is empty.\n\n💡 Stumps can be lucky or unlucky - this adds suspense!',
  ...
  'Во 2-й стадии, когда у игрока остаётся ОДНА карта — он ОБЯЗАН нажать кнопку «Одна карта!»\n\n💀 Если забыл объявить, штраф срабатывает только если кто-то за столом успел первым спросить «Сколько карт?».\n\n📌 Штраф: забывший получает по одной карте от всех остальных игроков за столом.\n\n🔍 Кнопка «Сколько карт?» загорается когда у кого-то подозрительно мало карт. Кто нажал первым — тот и запускает проверку.\n\n💡 Совет: следите за количеством карт у соперников!':
    'In Stage 2, when a player has ONE card left, they MUST press "One Card!".\n\n💀 If they forget, the penalty starts only if someone at the table asks "How many cards?" first.\n\n📌 Penalty: the player who forgot receives one card from every other player at the table.\n\n🔍 The "How many cards?" button lights up when someone has very few cards. Whoever taps it first starts the check.\n\n💡 Tip: watch how many cards the others have.',
