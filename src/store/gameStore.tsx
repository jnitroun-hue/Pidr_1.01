  ...
  persist(
    (set, get) => ({
      ...
      findAvailableTargets: (currentPlayerId: string) => {
        ...
        // ПРАВИЛО: Карта кладется НА КАРТУ НА 1 РАНГ ВЫШЕ
        // Пример: Дама (12) → Король (13), Валет (11) → Дама (12), 3 → 4
        // ИСКЛЮЧЕНИЯ: 
        // 1) Только двойка (2) может ложиться на Туз (14)!
        // 2) Туз (14) НЕ МОЖЕТ никуда ложиться (самая старшая карта)
        ...
        if (currentRank === 2) {
          ...
          // Туз НЕ МОЖЕТ никуда ложиться - это самая старшая карта!
          console.log(`🎯 [findAvailableTargets] ⚠️ Туз (14) не может ходить - самая старшая карта!`);
          return []; // Нет доступных целей
        } else {
          // Обычное правило: ищем карты на 1 ранг ВЫШЕ нашей карты
          // 3 → 4, 4 → 5, ..., Валет(11) → Дама(12), Дама(12) → Король(13), Король(13) → Туз(14)
